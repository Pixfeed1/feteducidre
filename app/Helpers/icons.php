...
/**
 * Helpers icônes — SVG inline depuis Lucide Icons
 * Zéro JavaScript, tout est rendu côté serveur
 */

// Tableau des paths SVG Lucide (viewBox 0 0 24 24)
const ICONS = [
    ...
    'drama'           => '<path d="M10 11h.01"/><path d="M14 6h.01"/><path d="M18 6h.01"/><path d="M6.5 13.1h.01"/><path d="M22 5c0 9-4 12-6 12s-6-3-6-12c0-2 2-3 6-3s6 1 6 3"/><path d="M17 14.672A6 6 0 0 1 2 10c0-4.97 4-8 6-8s3 .9 3 2"/>',
    'shopping-basket' => '<path d="m5 11 4-7"/><path d="m19 11-4-7"/><path d="M2 11h20"/><path d="m3.5 11 1.6 7.4a2 2 0 0 0 2 1.6h9.8a2 2 0 0 0 2-1.6l1.7-7.4"/><path d="m9 11 1 9"/><path d="M4.5 15.5h15"/><path d="m15 11-1 9"/>',
    ...
    'file-pen'        => '<path d="M12 22h6a2 2 0 0 0 2-2V7l-5-5H6a2 2 0 0 0-2 2v10"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10.4 12.6a2 2 0 0 0-3 3L12 21l4-1-1.6-5.4z"/>',
...
/**
 * Génère un SVG inline pour une icône Lucide
 *
 * @param string $name Nom de l'icône
 * @param int $size Taille en pixels
 * @param string $class Classes CSS additionnelles
 * @param string $stroke Couleur du trait
 * @param float $weight Épaisseur du trait
 */
function icon(string $name, int $size = 24, string $class = '', string $stroke = 'currentColor', float $weight = 2): string
{
    if (!isset(ICONS[$name])) {
        return '<!-- icône inconnue : ' . htmlspecialchars($name) . ' -->';
    }

    $classAttr = $class ? ' class="icon ' . htmlspecialchars($class) . '"' : ' class="icon"';
    ...
    return '<svg'
        ...
        . ' viewBox="0 0 24 24"'
        . ' fill="none"'
        . ' stroke="' . htmlspecialchars($stroke) . '"'
        . ' stroke-width="' . $weight . '"'
